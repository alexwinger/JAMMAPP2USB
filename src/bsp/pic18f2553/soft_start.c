...
#define APP_POWER_PWM_START 0x10
#define APP_POWER_PWM_END 0x50
#define APP_POWER_INCREMENT_PERIOD 0x0A
...
typedef union _app_power_status_bits
{
    struct
    {
        unsigned power_ready :1;
        unsigned ramping_power :1;
    };
    unsigned int Val;
...
/*************** Variables ******************/
APP_POWER_STATUS_BITS app_power_status;
/********************************************/
...
void AppPowerEnable(void)
{
    ...
    if(app_power_status.power_ready == 0)
    {
        ...
        //Set the FET to full off to start off with.  Since it is a P-MOSFET,
        //we drive it logic high to turn it off.
        APP_POWER_PIN_LAT = 1;
        ...
        CCP1CON = 0b00001111; //single output, PWM, active low (output is inverted from normal)
